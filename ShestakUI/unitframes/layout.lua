...
local function Shared(self, unit)
	...
	if (unit == "arena" and db_uf.show_arena == true and unit ~= "arenatarget") or (unit == "boss" and db_uf.show_boss == true) then
	...
	elseif unit == "arena" then
		if db_uf.arena_on_right == true then
	...
	if unit ~= "player" then
		...
		if unit ~= "arenatarget" then
			self.Level = SettingsDB.SetFontString(self.Power, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
		end
		if unit == "target" then
	...
	if unit == "player" then
		...
		self.FlashInfo.ManaLevel = SettingsDB.SetFontString(self.FlashInfo, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
		self.FlashInfo.ManaLevel:SetPoint("CENTER", 0, SettingsDB.Scale(1))
		
		-- Combat icon
		if db_uf.icons_combat == true then
		...
		-- Resting icon
		if db_uf.icons_resting == true and SettingsDB.level ~= MAX_PLAYER_LEVEL then
		...
		if db_rf.icons_leader == true then
			-- Leader icon
		...
		-- LFD role icons
		if db_rf.icons_lfd_role == true then 
		...
		-- Rune bar
		if db_uf.plugins_rune_bar == true and SettingsDB.class == "DEATHKNIGHT" then
		...
		-- Totem bar
		if db_uf.plugins_totem_bar == true and SettingsDB.class == "SHAMAN" then
			self.TotemBar = {}
			...
			for i = 1, 4 do
				...
				if db_uf.plugins_totem_bar_name == true then
					self.TotemBar[i].Name = SettingsDB.SetFontString(self.TotemBar[i], db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
		...
		if SettingsDB.class == "DRUID" then
			...
			self.DruidMana = SettingsDB.SetFontString(self.Power, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
			self.DruidMana:SetTextColor(1, 0.49, 0.04)
			
			if db_uf.plugins_eclipse_bar == true then
				...
				local eclipseBarText = SettingsDB.SetFontString(solarBar, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
				...
				local eclipseBarPers = SettingsDB.SetFontString(solarBar, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
		...
		if (SettingsDB.class == "WARLOCK" and db_uf.plugins_shard_bar == true) or (SettingsDB.class == "PALADIN" and db_uf.plugins_holy_bar == true) then
		...
		-- Vengeance bar
		if db_uf.plugins_vengeance_bar == true then
			...
			local vengeanceBarText = SettingsDB.SetFontString(statusBar, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
		...
		-- Experience bar
		if SettingsDB.level ~= MAX_PLAYER_LEVEL and db_uf.plugins_experience_bar == true then
			...
			if db_uf.portrait_enable == true then
				self.Experience:SetPoint("TOPLEFT", self, "TOPLEFT", SettingsDB.Scale(-25-db_uf.portrait_width), SettingsDB.Scale(28))
			else
		...
		-- Reputation bar
		if db_uf.plugins_reputation_bar == true then
		...
		-- Swing bar
		if db_uf.plugins_swing == true then
			...
			self.Swing.Text = SettingsDB.SetFontString(self.Swing, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
			self.Swing.Text:SetPoint("CENTER", 0, 0)
			self.Swing.Text:SetTextColor(1, 1, 1)
		end
		
		-- GCD spark
		if db_uf.plugins_gcd == true then
	...
	if unit == "player" or unit == "target" then
		if db_uf.portrait_enable == true then
			...
			self.Portrait:SetHeight(SettingsDB.Scale(db_uf.portrait_height))
			self.Portrait:SetWidth(SettingsDB.Scale(db_uf.portrait_width))
			if unit == "player" then
				self.Portrait:SetPoint(unpack(db_pf.player_portrait))
			elseif unit == "target" then
				self.Portrait:SetPoint(unpack(db_pf.target_portrait))
			end
			...
			if unit == "player" then
				...
				if db_uf.portrait_classcolor_border == true then
		...
		if unit == "player" then
			...
			self.Debuffs["growth-x"] = "LEFT"
			if (SettingsDB.class == "DEATHKNIGHT" and db_uf.plugins_rune_bar == true) 
			or (SettingsDB.class == "SHAMAN" and db_uf.plugins_totem_bar == true)
			or (SettingsDB.class == "DRUID" and db_uf.plugins_eclipse_bar == true) 
			or (SettingsDB.class == "PALADIN" and db_uf.plugins_holy_bar == true)
			or (SettingsDB.class == "WARLOCK" and db_uf.plugins_shard_bar == true) then
		...
		if unit == "target" then
			...
			if db_uf.icons_combo_point == true then
				local CPoints = {}
			...
			if db_uf.plugins_talents == true then
				self.Talents = SettingsDB.SetFontString(self.Power, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
				self.Talents:SetTextColor(1, 0, 0)
				self.Talents:SetPoint("BOTTOM", self.Power, "BOTTOM", 0, SettingsDB.Scale(-1))
			end
		end

		if db_uf.plugins_combat_feedback == true then
			self.CombatFeedbackText = SettingsDB.SetFontString(self.Health, db_ff.unit_frames_font, db_ff.unit_frames_font_size * 2, db_ff.unit_frames_font_style)
			if db_uf.portrait_enable == true then
		...
		if db_uf.icons_pvp == true then
			self.Status = SettingsDB.SetFontString(self.Health, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
	...
	if db_uf.unit_castbar == true and unit ~= "arenatarget" then
		...
		if unit == "player" then
			if db_uf.castbar_icon == true then
				self.Castbar:SetPoint(unpack(db_pf.player_castbar))
				self.Castbar:SetWidth(SettingsDB.Scale(258))
			else
				self.Castbar:SetPoint(db_pf.player_castbar[1], db_pf.player_castbar[2], db_pf.player_castbar[3], db_pf.player_castbar[4]-23, db_pf.player_castbar[5])
		...
		elseif unit == "target" then
			if db_uf.castbar_icon == true then
				if db_uf.plugins_swing == true then
					self.Castbar:SetPoint(unpack(db_pf.target_castbar))
				else
					self.Castbar:SetPoint(db_pf.target_castbar[1], db_pf.target_castbar[2], db_pf.target_castbar[3], db_pf.target_castbar[4], db_pf.target_castbar[5] - 12)
				end
				self.Castbar:SetWidth(SettingsDB.Scale(258))
			else
				if db_uf.plugins_swing == true then
					self.Castbar:SetPoint(db_pf.target_castbar[1], db_pf.target_castbar[2], db_pf.target_castbar[3], db_pf.target_castbar[4] + 23, db_pf.target_castbar[5])
				else
					self.Castbar:SetPoint(db_pf.target_castbar[1], db_pf.target_castbar[2], db_pf.target_castbar[3], db_pf.target_castbar[4] + 23, db_pf.target_castbar[5] - 12)
				end
				self.Castbar:SetWidth(SettingsDB.Scale(281))
			end
			self.Castbar:SetHeight(SettingsDB.Scale(16))
		...
		if unit == "focus" then
			...
			self.Castbar.Button:SetPoint(unpack(db_pf.focus_castbar))
			...
			self.Castbar.Time = SettingsDB.SetFontString(self.Castbar, db_ff.unit_frames_font, db_ff.unit_frames_font_size * 2, db_ff.unit_frames_font_style)
		...
		if unit == "player" or unit == "target" or unit == "arena" or unit == "boss" then
			self.Castbar.Time = SettingsDB.SetFontString(self.Castbar, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
			...
			self.Castbar.Text = SettingsDB.SetFontString(self.Castbar, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
			...
			if db_uf.castbar_icon == true and unit ~= "arena" then
				self.Castbar.Button = CreateFrame("Frame", nil, self.Castbar)
			...
			if unit == "player" and db_uf.castbar_latency == true then
				...
				self.Castbar.Latency = SettingsDB.SetFontString(self.Castbar, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)
	...
	if db_uf.show_arena and unit == "arena" then
		...
		self.AuraTracker.text = SettingsDB.SetFontString(self.AuraTracker, db_ff.unit_frames_font, db_ff.unit_frames_font_size * 2, db_ff.unit_frames_font_style)
		self.AuraTracker.text:SetPoint("CENTER", self.AuraTracker, 0, 0)
		self.AuraTracker:SetScript("OnUpdate", SettingsDB.AuraTrackerTime)
	end
	
	if db_rf.aggro_border == true and unit ~= "arenatarget" then
...
if SettingsCF.actionbar.bottombars == 1 then
	player:SetPoint(db_pf.player[1], db_pf.player[2], db_pf.player[3], SettingsDB.Scale(db_pf.player[4]), SettingsDB.Scale(db_pf.player[5] - 28))
	target:SetPoint(db_pf.target[1], db_pf.target[2], db_pf.target[3], SettingsDB.Scale(db_pf.target[4]), SettingsDB.Scale(db_pf.target[5] - 28))
elseif SettingsCF.actionbar.bottombars == 2 then
	player:SetPoint(unpack(db_pf.player))
	target:SetPoint(unpack(db_pf.target))
elseif SettingsCF.actionbar.bottombars == 3 then
	player:SetPoint(db_pf.player[1], db_pf.player[2], db_pf.player[3], SettingsDB.Scale(db_pf.player[4]), SettingsDB.Scale(db_pf.player[5] + 28))
	target:SetPoint(db_pf.target[1], db_pf.target[2], db_pf.target[3], SettingsDB.Scale(db_pf.target[4]), SettingsDB.Scale(db_pf.target[5] + 28))
end

if db_uf.show_pet == true then
	local pet = oUF:Spawn("pet", "oUF_Pet")
	pet:SetPoint(unpack(db_pf.pet))
	pet:SetSize(SettingsDB.Scale(105), SettingsDB.Scale(16))
end

if db_uf.show_focus == true then
	local focus = oUF:Spawn("focus", "oUF_Focus")
	focus:SetPoint(unpack(db_pf.focus))
	focus:SetSize(SettingsDB.Scale(105), SettingsDB.Scale(16))

	local focustarget = oUF:Spawn("focustarget", "oUF_FocusTarget")
	focustarget:SetPoint(unpack(db_pf.focus_target))
	focustarget:SetSize(SettingsDB.Scale(105), SettingsDB.Scale(16))
else
	local focus = oUF:Spawn("focus", "oUF_Focus")
end

if db_uf.show_target_target == true then
	local targettarget = oUF:Spawn("targettarget", "oUF_TargetTarget")
	targettarget:SetPoint(unpack(db_pf.target_target))
	targettarget:SetSize(SettingsDB.Scale(105), SettingsDB.Scale(16))
end

if db_uf.show_boss == true then
	local boss = {}
	for i = 1, MAX_BOSS_FRAMES do
		boss[i] = oUF:Spawn("boss"..i, "oUF_Boss"..i)
		if i == 1 then
			if db_uf.boss_on_right == true then
				boss[i]:SetPoint(unpack(db_pf.boss))
			else
				boss[i]:SetPoint("BOTTOMLEFT", db_pf.boss[2], "LEFT", SettingsDB.Scale(db_pf.boss[4] + 38), SettingsDB.Scale(db_pf.boss[5]))
			end
		else
			boss[i]:SetPoint("BOTTOM", boss[i-1], "TOP", 0, SettingsDB.Scale(30))
		end
		boss[i]:SetSize(SettingsDB.Scale(150), SettingsDB.Scale(27))
	end
end

if db_uf.show_arena == true then
	local arena = {}
	for i = 1, 5 do
		arena[i] = oUF:Spawn("arena"..i, "oUF_Arena"..i)
		if i == 1 then
			if db_uf.arena_on_right == true then
				arena[i]:SetPoint(unpack(db_pf.arena))
			else
				arena[i]:SetPoint("BOTTOMLEFT", db_pf.arena[2], "LEFT", SettingsDB.Scale(db_pf.arena[4] + 75), SettingsDB.Scale(db_pf.arena[5]))
			end
		else
	...
	if(oUF) then
		for i, v in pairs(oUF.units) do
			if not v.fff then
				...
				v.fffs = SettingsDB.SetFontString(v.fff, db_ff.unit_frames_font, db_ff.unit_frames_font_size, db_ff.unit_frames_font_style)

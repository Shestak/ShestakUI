...
local StartTimer = function(sGUID, sID)
	...
	local locClass = "UNKNOWN"
	locClass, _, _, _, _, name, _ = GetPlayerInfoByGUID(sGUID)
	local color = (CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS)[string.upper(locClass):gsub("%s+", "")]
	if color then
